...
class XLMRobertaForTokenClassification(RobertaPreTrainedModel):				# This class is part of transformers alread - learning exercise.
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, labels=None,  **kwargs):
        ...
        base_model_output = self.roberta(input_ids=input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids, **kwargs)
 
        # Without [0]: TypeError: dropout(): argument 'input' (position 1) must be Tensor, not BaseModelOutputWithPoolingAndCrossAttentions.
        # So forward() in RobertaModel returns BaseModelOutputWithPoolingAndCrossAttentions and not a tensor.
        cvs = self.dropout(base_model_output[0])								
